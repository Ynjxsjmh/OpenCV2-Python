...
目录 =./used_for_tg_sticker= 是用于将图片格式的宽转化为 512px 以满足 tg 表情包的长或宽有一个必须为 512px 的限制。其下的 expand_img.py 的用处就是将图片的宽转化为 512px。而 =change_file_name.ps1= 是目的是将 =猥琐萌0001.png= 该成 =0001.png=，因为我试了下 cv2.imread 貌似不能读进带有中文的图片。这两个程序都得和你需要处理图片放在一起。子目录 =CuteSmallBoy= 存放的是 =猥琐萌表情包=，其来源是 [[http://tieba.baidu.com/home/main?un=LONE04&fr=home&id=65bc4c4f4e453034092b][百度贴吧 Lone04]]。
** 后话
